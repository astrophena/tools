...
import (
	...
	"go.astrophena.name/tools/internal/starlark/docgen/ast"
	"go.astrophena.name/tools/internal/starlark/docgen/docstring"
	"go.astrophena.name/tools/internal/starlark/docgen/symbols"
	"go.astrophena.name/tools/internal/util/set"
)
...
type symbol struct {
	...
	tags set.Set[string]   // lazily extracted from "DocTags" remarks section
}
...
func (s *symbol) Symbols(flavors ...string) (out []*symbol, err error) {
	...
	keepFlavors := set.NewFromSlice(flavors...)
