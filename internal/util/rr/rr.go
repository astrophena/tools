...
// only in test programs (built by “go test”). See the [Open] documentation
// for more details.
package rr
...
// A RecordReplay is an [http.RoundTripper] that can operate in two modes:
// record and replay.
//
...
// the RecordReplay calls each scrub function, in the order they were
// registered, to canonicalize non-deterministic parts of the request and remove
// secrets.
//
// Scrubbing only applies to a copy of the request used in the record/replay
// log; the unmodified original request is sent to the actual server in
// recording mode.  A scrub function can assume that if req.Body is not nil,
...
// Open opens a new record/replay log in the named file and returns a
// [RecordReplay] backed by that file.
//
// By default Open expects the file to exist and contain a previously-recorded
// log of (request, response) pairs, which [RecordReplay.RoundTrip] consults to
// prepare its responses.
//
// If the command-line flag -httprecord is set to a non-empty regular expression
// that matches file, then Open creates the file as a new log. In that mode,
// [RecordReplay.RoundTrip] makes actual HTTP requests using rt but then logs
// the requests and responses to the file for replaying in a future run.
...
func open(file string, rt http.RoundTripper) (*RecordReplay, error) {
	// Note: To handle larger traces without storing entirely in memory, could
	// instead read the file incrementally, storing a map[hash]offsets and then
	// reread the relevant part of the file during RoundTrip.
...
// Client returns an http.Client using rr as its transport. It is a shorthand
// for:
//
//	return &http.Client{Transport: rr}
//
// For more complicated uses, use rr or the [RecordReplay.RoundTrip] method
// directly.
func (rr *RecordReplay) Client() *http.Client {
	return &http.Client{Transport: rr}
}

// A Body is an io.ReadCloser used as an HTTP request body. In a Scrubber, if
// req.Body != nil, then req.Body is guaranteed to have type *Body, making it
// easy to access the body to change it.
type Body struct {
	Data       []byte
	ReadOffset int
}
...
//
// If rr has been opened in record mode, RoundTrip passes the requests on to the
// RoundTripper specified in the call to [Open] and then logs the (request,
// response) pair to the underlying file.
//
// If rr has been opened in replay mode, RoundTrip looks up the request in the
// log and then responds with the previously logged response.
//
