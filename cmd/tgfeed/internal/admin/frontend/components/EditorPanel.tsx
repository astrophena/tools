...
class ErrorWidget extends WidgetType {
  ...
  override eq(other: ErrorWidget) { return other.message === this.message; }
  override toDOM() {
...
const setInlineError = StateEffect.define<{line: number, message: string} | null>();
      ...
      if (e.is(setInlineError)) {
        if (!e.value) {
          ...
          if (line > 0 && line <= tr.state.doc.lines) {
            ...
            builder.add(lineInfo.from, lineInfo.to > lineInfo.from ? lineInfo.to : lineInfo.from, Decoration.mark({
              class: "cm-error-line",
            }));
      ...
      <CodeMirror
        ...
        onChange={(value) => {
          ...
          if (editorRef.current?.view) {
            editorRef.current.view.dispatch({ effects: setInlineError.of(null) });
          }
        }}
      />
      {resource.error && (
