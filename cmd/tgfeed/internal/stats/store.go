...
func (s *Store) ListRunSummaries(ctx context.Context, limit int, beforeStartedAt *int64) ([]RunSummary, error) {
	...
	COALESCE(CAST(json_extract(payload_json, '$.fetch_retries_total') AS INTEGER), 0) AS fetch_retries_total,
	COALESCE(CAST(json_extract(payload_json, '$.feeds_retried_count') AS INTEGER), 0) AS feeds_retried_count,
...
FROM runs`
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&item.MessagesSent,
			&item.MessagesFailed,
			&item.FetchRetriesTotal,
			&item.FeedsRetriedCount,
